...
const AdminDashboard = () => {
  ...
  const [form, setForm] = useState({ name: '', email: '', role: 'worker', password: '' });
  ...
  const onCreate = async (e) => {
    ...
    setTempPw('');
    try {
      const res = await axios.post(`${API}/api/admin/users`, form, { headers: { ...authHeader, 'Content-Type': 'application/json' } });
      setTempPw(res.data.temporaryPassword); // reveal once
      setForm({ name: '', email: '', role: 'worker', password: '' });
      fetchUsers();
    } catch (e) {
      console.error(e);
            ...
            <table className="table">
              ...
              <tbody>
                ...
                {users.length === 0 && !listLoading && (
                  <tr>
                    <td colSpan={5} className="text-center text-base-content/60">No users found</td>
                  </tr>
                )}
              </tbody>
            </table>
