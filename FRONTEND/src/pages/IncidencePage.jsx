...
const IncidencePage = () => {
  ...
  const handleDelete = async (id, status, e) => {
    ...
    if (status !== 'Resolved') {
      setInfoMessage('Only resolved incidence reports can be deleted. Please resolve the report first.');
      setTimeout(() => setInfoMessage(''), 4000);
      return;
    }
    
    // Confirm deletion
    if (!window.confirm('Are you sure you want to delete this resolved incidence report? This action cannot be undone.')) {
      return;
    }
    ...
    try {
      ...
      setError('Failed to delete incidence report. Please try again.');
    } finally {
      setDeletingId(null);
    }
  };
