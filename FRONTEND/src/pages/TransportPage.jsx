...
const TransportPage = () => {
  ...
  const fetchTransports = async () => {
    try {
      ...
      Swal.fire('Error', 'Failed to load transports', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredTransports = transports.filter(transport => {
    const matchesSearch = transport.vehicleId.toLowerCase().includes(searchText.toLowerCase()) || 
                         transport.driverName.toLowerCase().includes(searchText.toLowerCase());
      ...
      <div className="min-h-screen bg-base-200">
        <div className="container mx-auto p-4 flex justify-center items-center h-64">
          <div className="text-center">
            ...
            <p className="mt-3">Loading transports...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="container mx-auto p-4">
        ...
        {error && (
          <div className="alert alert-error mb-4">
            <span>{error}</span>
            <button className="btn btn-sm btn-ghost" onClick={fetchTransports}>
              Try Again
            </button>
          </div>
        )}
