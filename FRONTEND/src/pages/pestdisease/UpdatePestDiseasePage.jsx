...
import { 
  ...
  FileText, CheckCircle, Map, Ruler, Loader 
} from 'lucide-react';
...
const UpdatePestDiseasePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fieldsResponse = await axios.get(`${API}/api/fields`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`${API}/api/pest-diseases/${id}`, form, {
        headers: { 
  ...
  return (
    <div className="min-h-screen bg-green-50 py-10 px-4">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-md p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-green-800 mb-2">Update Pest/Disease Report</h1>
